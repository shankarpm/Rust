API_URL = 'http://10.195.176.70:8000/apiportal/' # TODO
...
app.title = APP_HEADER
app._favicon = ('nyl.png')
server = app.server
...
UPLOAD_FILE_SIZE_LIMIT_BYTES = 15000000
...
def submit_chatbot_copy_response(chatbot_copy_response_n_clicks, chatbot_text_card_value, chatbot_user_text_card_value, chatbot_text_card_file_name_store_data):
    ...
    if 'chatbot-copy-results' in changed_id:
        generate_log(tz_eastern, start_time, transferDateTime, timestamp_id, 'chatbot', 'copied',
            original_filename, new_filename, str(chatbot_user_text_card_value[0]), str(chatbot_text_card_value[0]))
...
# Generate log files
###############################################################################
def generate_log(tz_eastern, start_time, transferDateTime, timestamp_id, mode, submit_type, original_filename, new_filename, user_input, results_paragraph, results_json_from_api=''):
    try:
        ...
        log_list = [
            request.headers.get('domino-username'), # Domino username; None if on local
            timestamp_id, # YYYYMMDDHHMMSS
            ptime, # Processing time
            'Success', # Success (or Fail for exception log)
            mode, # summarizer or chatbot
            submit_type, # submitted, copied, good-response, or bad-response
            original_filename, # Original file name ending with .pdf
            new_filename, # New file name (username-timestamp including milliseconds) ending with .pdf
            user_input, # Prompt entered to the chatbot, empty string for summarizer
            results_paragraph, # Answer from either the summarizer or the chatbot
            results_json_from_api, # More detailed results in JSON format passed back from the API
...
def get_results(submit_button_n_clicks, task_dropdown_label, single_file_upload_estimate_store):
    ...
    if 'submit-button' in changed_id:
        ...
        generate_log(tz_eastern, start_time, transferDateTime, timestamp_id, 'summarizer', 'submitted',
                     summarizer_last_submit_store_data[0], summarizer_last_submit_store_data[1], '', str(results_paragraph), str(results_json_from_api))
...
def submit_copy_response(good_response_button_n_clicks, results_paragraph, summarizer_last_submit_store_data):
    ...
    if 'copy-results' in changed_id:
        generate_log(tz_eastern, start_time, transferDateTime, timestamp_id, 'summarizer', 'copied',
                     summarizer_last_submit_store_data[0], summarizer_last_submit_store_data[1], '', str(results_paragraph))
    return dash.no_update
...
def run_chatbot(n_clicks, user_input, task_dropdown_label, store_conversation_file_name_data, single_file_upload_estimate_store, chatbot_submit_button_n_clicks_store):
    ...
    generate_log(tz_eastern, start_time, transferDateTime, timestamp_id, 'chatbot', 'submitted',
            original_filename, new_filename, str(user_input), str(results_paragraph), str(results_json_from_api))
