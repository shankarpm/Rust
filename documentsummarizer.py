...
#DOMINO_RUN_ID = os.environ.get('DOMINO_RUN_ID')
 ...
 #   RUN_ON_LOCAL = 0
#if RUN_ON_LOCAL:
app = dash.Dash(__name__,requests_pathname_prefix='/documentsummarizer/',meta_tags=[
...
def get_results(submit_button_n_clicks, task_dropdown_label, single_file_upload_estimate_store):
    ...
    if 'submit-button' in changed_id:
        ...
        #print(str(results_paragraph))
        # results_paragraph = "The Beige Book provides an overview of economic activity in the United States as   The Beige Book provides an overview of economic activity in the United States as"
        #results_paragraph_title = 'test data'
        #time.sleep(2)
        return None, {'font-size': 'max(0.82vw, 10.5px)', 'whiteSpace': 'pre-wrap'}, {'font-size': 'max(0.675vw, 8.75px)'}, {}, str(results_paragraph), results_paragraph_title, dash.no_update, summarizer_last_submit_store_data, single_file_upload_estimate_store
...
dash.clientside_callback(
    ...
    function updateTypewriter(children) {
        try {
            ...
            function typeWriter() {
                if (!continueTyping) {
                    console.log("continue typing" + continueTyping) 
                    ...
                    return window.dash_clientside.no_update;// #smp
                }
                if (i < txt.length) {
